...
def plot_forecasts_for_all_currencies(df_exchange_rate, df_new_dataset, pipeline_original_model, pipeline_new_model, plot_path):
    ...
    for i in range(8):
        ...
        df_currency_true = df_currency.iloc[-30:]
        df_currency_context = df_currency.iloc[-220:-30].reset_index(drop=True)
        
        # Get the positions mod 8 + i for the new dataset
        positions_mod_8 = np.arange(i, 240, 8)
        ### FIX BUG ###
        new_forecast = make_forecast(pipeline_new_model, df_new_dataset.iloc[-1920:-240], prediction_length=240, num_samples=20)
